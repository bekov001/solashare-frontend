    ...
    <Link href={`/assets/${asset.id}`}
      ...
      <div className="p-7">
        <div className="flex justify-between items-start mb-3">
          <div>
            <h3 className="text-lg font-black" style={{ color: "var(--text)" }}>{asset.title}</h3>
          ...
          <span className="font-black text-[#00693e] dark:text-[#14F195]">
            ...
            <span className="text-[10px] font-bold ml-1" style={{ color: "var(--text-faint)" }}>/ share</span>
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="rounded-2xl p-3" style={{ background: "var(--surface-low)" }}>
            ...
            <p className="text-sm font-bold" style={{ color: "var(--text)" }}>{formatNumber(asset.capacity_kw)} kW</p>
          </div>
          <div className="rounded-2xl p-3" style={{ background: "var(--surface-low)" }}>
            <p className="label-xs mb-1">Min. buy</p>
            <p className="text-sm font-bold" style={{ color: "var(--text)" }}>{formatUSDC(asset.price_per_share_usdc * 5)}</p>
          </div>
        </div>
...
export function AssetCardSkeleton() {
  return (
    <div className="rounded-[40px] overflow-hidden border animate-pulse" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
