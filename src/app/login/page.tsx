...
export default function LoginPage() {
  ...
  const [telegramPreview, setTelegramPreview] =
    useState<TelegramAuthPreview | null>(null);
      ...
      .catch((err) => {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to validate Telegram session.",
        );
      })
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-16 animate-fade-in">
      <div className="w-full max-w-md space-y-6">
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="input-new"
                placeholder={
                  mode === "login" ? "Your password" : "Minimum 8 characters"
                }
                type="password"
                value={password}
            ...
            {telegramPreviewLoading ? (
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs" style={{ color: "var(--text-faint)" }}>
                  Остальные методы входа ниже остаются доступны. После входа
                  через Telegram можно добавить email/password в профиле и
                  заходить в тот же аккаунт из обычного браузера.
                </p>
              </div>
            ) : (
              <p className="text-sm" style={{ color: "var(--text-muted)" }}>
        ...
        <p
          ...
          style={{ color: "var(--text-faint)" }}
        >
          Email/password stays available everywhere. Telegram sign-in is only
          offered when the page is opened inside Telegram.
        </p>
      </div>
    </div>
  );
