  ...
  if (!user) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-24 text-center animate-fade-in">
        ...
        <h1 className="text-3xl font-black mb-3" style={{ color: "var(--text)" }}>Admin Panel</h1>
        <p className="text-sm mb-8" style={{ color: "var(--text-muted)" }}>Sign in as admin to manage the platform.</p>
        <button onClick={() => devSwitchRole("admin")} className="btn-sol px-8">
          Demo Login as Admin
        </button>
      </div>
    );
  ...
  if (user.role !== "admin") {
    return (
      <div className="max-w-xl mx-auto px-6 py-24 text-center" style={{ color: "var(--text-muted)" }}>
        ...
        <button onClick={() => devSwitchRole("admin")} className="text-[#9945FF] underline font-bold">Switch role</button>
      </div>
    );
  }
    ...
    <div className="max-w-[1440px] mx-auto px-8 py-10 animate-fade-in space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className="text-3xl font-black" style={{ color: "var(--text)" }}>Admin Panel</h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total Assets",   val: assets.length,  color: "text-[var(--text)]" },
          { label: "Pending Review", val: pending.length, color: "text-[#9945FF]" },
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-black text-sm truncate" style={{ color: "var(--text)" }}>{a.title}</p>
                        <p className="text-xs" style={{ color: "var(--text-faint)" }}>{a.id}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <Link href={`/assets/${a.id}`} className="p-1.5 rounded-xl transition-colors hover:bg-[#9945FF]/5"
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                  ...
                  <tr key={i} className="border-b" style={{ borderColor: "var(--border)" }}>
                    {[1,2,3,4].map(j => (
                      <td key={j} className="px-5 py-4">
                        <div className="h-3 rounded-xl animate-pulse w-20" style={{ background: "var(--surface-low)" }} />
                      </td>
                    ))}
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={4} className="px-5 py-10 text-center" style={{ color: "var(--text-faint)" }}>No audit events.</td>
                </tr>
              ) : logs.map(log => (
                <tr key={log.id} className="border-b transition-colors hover:bg-[#9945FF]/5"
                    style={{ borderColor: "var(--border)" }}>
                  <td className="px-5 py-3.5 text-xs font-mono whitespace-nowrap" style={{ color: "var(--text-muted)" }}>
                    {formatDate(log.created_at)}
                  </td>
                  <td className="px-5 py-3.5 text-xs font-mono" style={{ color: "var(--text-faint)" }}>
                    {log.actor_user_id.slice(0, 8)}…
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="text-xs capitalize" style={{ color: "var(--text-muted)" }}>{log.entity_type}</span>
                    <span className="text-xs" style={{ color: "var(--text-faint)" }}> · {log.entity_id.slice(0, 8)}…</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className="px-2 py-1 rounded-lg text-xs font-mono text-[#9945FF]"
                          style={{ background: "#9945FF10" }}>
                      {log.action}
                    </span>
                  </td>
