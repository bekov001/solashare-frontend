...
export default function NewAssetPage() {
  ...
  const [info, setInfo] = useState({
    ...
    energy_type: "solar", location_country: "Kazakhstan",
    location_city: "", capacity_kw: "",
  });

  // Step 2 - Sale terms
  const [sale, setSale] = useState({
    valuation_usdc: "", total_shares: "", price_per_share_usdc: "",
    minimum_buy_amount_usdc: "", target_raise_usdc: "",
  });

  // Step 3 - Documents
  const [doc, setDoc] = useState({
    type: "technical_passport", title: "", storage_provider: "arweave",
  ...
  if (!user) {
    return (
      <div className="max-w-xl mx-auto px-6 py-24 text-center animate-fade-in">
        ...
        <button onClick={() => devSwitchRole("issuer")} className="btn-primary">
  ...
  async function handleInfoSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // demo fallback
      setAssetId("demo-asset-" + Date.now());
      setStep("sale");
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10 animate-fade-in">
          ...
          return (
            <div key={s.key} className="flex items-center gap-2 flex-1">
              <div className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold transition-colors ${
                current ? "bg-emerald-600/20 border border-emerald-700/60 text-emerald-400" :
              ...
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-px ${done ? "bg-emerald-800/60" : "bg-surface-200/40"}`} />
              )}
            </div>
          );
        })}
      ...
      {step === "info" && (
        <form onSubmit={handleInfoSubmit} className="glass-card p-7 space-y-5">
          ...
          <div>
            ...
            <input required className="input-field" placeholder="e.g. Almaty Solar Farm A"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select required className="input-field"
                value={info.energy_type} onChange={e => setInfo(p => ({ ...p, energy_type: e.target.value }))}>
            ...
            <div>
              ...
              <input required type="number" min="1" className="input-field" placeholder="150"
                value={info.capacity_kw} onChange={e => setInfo(p => ({ ...p, capacity_kw: e.target.value }))} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required className="input-field" placeholder="Kazakhstan"
                value={info.location_country} onChange={e => setInfo(p => ({ ...p, location_country: e.target.value }))} />
            </div>
            <div>
              <label className="label-text block mb-2">City *</label>
              <input required className="input-field" placeholder="Almaty"
                value={info.location_city} onChange={e => setInfo(p => ({ ...p, location_city: e.target.value }))} />
            </div>
          </div>

          <div>
            <label className="label-text block mb-2">Short Description *</label>
            <input required className="input-field" placeholder="Yield-bearing solar asset in…"
              value={info.short_description} onChange={e => setInfo(p => ({ ...p, short_description: e.target.value }))} />
          </div>

          <div>
            <label className="label-text block mb-2">Full Description</label>
            <textarea rows={4} className="input-field resize-none" placeholder="Detailed description of the installation, legal setup, power purchase agreement…"
              value={info.full_description} onChange={e => setInfo(p => ({ ...p, full_description: e.target.value }))} />
          </div>
        ...
        <form onSubmit={handleSaleSubmit} className="glass-card p-7 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required type="number" min="1" className="input-field" placeholder="100000"
                value={sale.valuation_usdc} onChange={e => setSale(p => ({ ...p, valuation_usdc: e.target.value }))} />
            </div>
            <div>
              <label className="label-text block mb-2">Total Shares *</label>
              <input required type="number" min="1" className="input-field" placeholder="10000"
                value={sale.total_shares} onChange={e => setSale(p => ({ ...p, total_shares: e.target.value }))} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required type="number" min="0" step="0.01" className="input-field" placeholder="10.00"
                value={sale.price_per_share_usdc} onChange={e => setSale(p => ({ ...p, price_per_share_usdc: e.target.value }))} />
            </div>
            <div>
              <label className="label-text block mb-2">Min. Buy (USDC) *</label>
              <input required type="number" min="0" step="0.01" className="input-field" placeholder="50.00"
                value={sale.minimum_buy_amount_usdc} onChange={e => setSale(p => ({ ...p, minimum_buy_amount_usdc: e.target.value }))} />
            </div>
          </div>

          <div>
            <label className="label-text block mb-2">Target Raise (USDC) *</label>
            <input required type="number" min="1" className="input-field" placeholder="50000"
              value={sale.target_raise_usdc} onChange={e => setSale(p => ({ ...p, target_raise_usdc: e.target.value }))} />
          </div>
        ...
        <form onSubmit={handleDocSubmit} className="glass-card p-7 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select required className="input-field"
                value={doc.type} onChange={e => setDoc(p => ({ ...p, type: e.target.value }))}>
            ...
            <div>
              ...
              <select className="input-field"
                value={doc.storage_provider} onChange={e => setDoc(p => ({ ...p, storage_provider: e.target.value }))}>
                {["arweave","ipfs","s3"].map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
          </div>
          ...
          <div>
            ...
            <input required className="input-field" placeholder="Technical Passport 2024"
              value={doc.title} onChange={e => setDoc(p => ({ ...p, title: e.target.value }))} />
          </div>

          <div>
            <label className="label-text block mb-2">Storage URI *</label>
            <input required className="input-field" placeholder="https://arweave.net/..."
              value={doc.storage_uri} onChange={e => setDoc(p => ({ ...p, storage_uri: e.target.value }))} />
          </div>

          <div>
            <label className="label-text block mb-2">Content Hash</label>
            <input className="input-field font-mono text-xs" placeholder="sha256:..."
              value={doc.content_hash} onChange={e => setDoc(p => ({ ...p, content_hash: e.target.value }))} />
          </div>
      ...
      {step === "done" && (
        <div className="glass-card p-10 text-center space-y-6">
          ...
          {assetId && (
            <div className="rounded-xl bg-surface-100/50 border border-surface-200/40 px-4 py-3 text-xs font-mono text-emerald-400">
              Asset ID: {assetId}
            </div>
          )}
