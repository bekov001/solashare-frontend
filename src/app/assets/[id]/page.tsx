...
export default function AssetDetailPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 animate-fade-in">
      {/* Back */}
      <Link href="/assets" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-emerald-400 transition-colors mb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-start justify-between gap-4 mb-4 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h1 className="text-2xl font-extrabold text-slate-100">{asset.title}</h1>
                </div>
              </div>
              <StatusBadge status={asset.status} />
            </div>

            <p className="text-slate-400 text-sm leading-relaxed mb-5">{asset.short_description}</p>
          ...
          <div className="glass-card overflow-hidden">
            <div className="flex border-b border-surface-200/40">
              {TABS.map(t => (
                <button
                  key={t.key}
            ...
            <div className="p-6">
              ...
              {tab === "overview" && (
                <div className="space-y-5">
                  <div>
                    <h3 className="font-semibold text-slate-200 mb-2">About this asset</h3>
                    <p className="text-slate-400 text-sm leading-relaxed">{asset.full_description}</p>
                  </div>

                  {asset.onchain_refs.onchain_asset_pubkey && (
                    <div>
                      <h3 className="font-semibold text-slate-200 mb-3">On-chain references</h3>
                      <div className="space-y-2">
                        {[
                          ...
                          <div key={r.label} className="flex items-center justify-between p-3 rounded-lg bg-surface-100/40 border border-surface-200/40">
                            <span className="text-xs text-slate-500">{r.label}</span>
                            <span className="font-mono text-xs text-emerald-400">{shortAddress(r.val!)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              {tab === "documents" && (
                <div className="space-y-3">
                  {documents.length === 0 ? (
                    ...
                    <div key={doc.id} className="flex items-center justify-between p-4 rounded-xl border border-surface-200/50 hover:border-emerald-800/50 transition-colors group">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-lg bg-surface-100 border border-surface-200/40 flex items-center justify-center">
                      ...
                      <a href={doc.storage_uri} target="_blank" rel="noopener noreferrer"
                         className="p-2 rounded-lg hover:bg-emerald-950/40 text-slate-500 hover:text-emerald-400 transition-colors">
              ...
              {tab === "revenue" && (
                <div className="space-y-3">
                  {revenue.length === 0 ? (
                    ...
                    <div key={epoch.id} className="p-4 rounded-xl border border-surface-200/50 space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-semibold text-slate-200">Epoch #{epoch.epoch_number}</span>
                        <span className={`text-xs px-2 py-1 rounded-md font-semibold ${
                          epoch.posting_status === "posted" ? "text-emerald-400 bg-emerald-950/50" :
                          epoch.posting_status === "settled" ? "text-sky-400 bg-sky-950/50" :
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <div className="text-center">
                          <p className="text-slate-500 text-xs">Gross</p>
                          <p className="text-slate-200 text-sm font-semibold">{formatUSDC(epoch.gross_revenue_usdc)}</p>
                        </div>
                        <div className="text-center">
                          <p className="text-slate-500 text-xs">Net</p>
                          <p className="text-slate-200 text-sm font-semibold">{formatUSDC(epoch.net_revenue_usdc)}</p>
                        </div>
                        <div className="text-center">
                          <p className="text-amber-500 text-xs">Distributable</p>
                          <p className="text-amber-400 text-sm font-bold">{formatUSDC(epoch.distributable_revenue_usdc)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              {tab === "holders" && holders && (
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { label: "Total Investors",    val: formatNumber(holders.total_investors),              color: "text-emerald-400" },
                    { label: "Funded",             val: formatPercent(holders.funded_percent),               color: "text-amber-400" },
                    { label: "Total Distributed",  val: formatUSDC(holders.total_distributed_usdc),          color: "text-sky-400" },
                    { label: "Total Claimed",      val: formatUSDC(holders.total_claimed_usdc),              color: "text-purple-400" },
                  ].map(s => (
                    <div key={s.label} className="p-4 rounded-xl bg-surface-100/50 border border-surface-200/40">
        ...
        <div className="space-y-5">
          ...
          {asset.sale_terms && (
            <div className="glass-card p-6 space-y-4">
              <h3 className="font-bold text-slate-100">Sale Terms</h3>

              {/* Funded bar */}
              <div>
                <div className="flex justify-between text-xs text-slate-500 mb-2">
                  <span>Funded</span>
                  <span className="text-emerald-400 font-semibold">{formatPercent(fundedPct)}</span>
                </div>
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  <div key={row.label} className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-slate-200 font-semibold">{row.val}</span>
                  </div>
                ))}
              </div>
          ...
          {asset.status === "active_sale" && (
            <div className="glass-card p-6 space-y-4">
              <h3 className="font-bold text-slate-100">Invest</h3>
              ...
              {quote && (
                <div className="rounded-xl bg-emerald-950/30 border border-emerald-900/50 p-3 space-y-1.5 text-sm">
                  <div className="flex justify-between">
                    <span className="text-slate-400">Shares to receive</span>
                    <span className="text-emerald-400 font-bold">{formatNumber(quote.shares_to_receive)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Fees</span>
                    <span className="text-slate-300">{formatUSDC(quote.fees_usdc)}</span>
                  </div>
                </div>
              )}

              {investMsg && (
                <div className="rounded-xl bg-sky-950/30 border border-sky-900/50 p-3 text-xs text-sky-300">
                  {investMsg}
                </div>
              )}
              ...
              <p className="text-xs text-slate-600 text-center">
                Gasless first transaction · Powered by Solana
              </p>
            </div>
          )}
          ...
          <div className="glass-card p-5">
            <h3 className="font-semibold text-slate-200 mb-3 text-sm">Revenue Summary</h3>
            <div className="grid grid-cols-2 gap-3">
              <div className="text-center p-3 rounded-xl bg-surface-100/50">
                <p className="text-2xl font-bold text-emerald-400">{asset.revenue_summary.total_epochs}</p>
                <p className="text-xs text-slate-500 mt-0.5">Total Epochs</p>
              </div>
              <div className="text-center p-3 rounded-xl bg-surface-100/50">
                <p className="text-2xl font-bold text-amber-400">{asset.revenue_summary.last_posted_epoch}</p>
                <p className="text-xs text-slate-500 mt-0.5">Last Epoch</p>
              </div>
            </div>
          </div>
        </div>
